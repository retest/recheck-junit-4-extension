...
if [ ! -f ${TRAVIS_BUILD_DIR}/already_signed ]; then
    ...
    openssl aes-256-cbc -K "${encrypted_eeb5d6c957a3_key}" -iv "${encrypted_eeb5d6c957a3_iv}" -in "${TRAVIS_BUILD_DIR}/retest-gmbh-gpg.asc.enc" -out "${TRAVIS_BUILD_DIR}/retest-gmbh-gpg.asc" -d
